...
# ----------------------------------------------------
# 0. Task 1 / Task 2 공식과 완전히 동일한 평가 함수
# ----------------------------------------------------
def evaluate_score_general(
    ...
    """
    Task 1, Task 2 공식과 완전히 동일한 평가 함수 (전체 데이터 기준 L/P 반 갈라서 200개씩 선택)

    Task 1:
        ROC-AUC Score

    Task 2:
        Total Net Profit = 100 * Σ I(d_i=True) * I(y_i=Good)
                           - 2000 * Σ I(d_i=True) * I(y_i=NG)

        Total = sqrt(
                    max(ROC-AUC - 0.5, 0) / 0.5
                    *
                    max(Total Net Profit, 0) / 20000
                )
    """
    ...
    # --- Task 1: ROC-AUC Score (Good=1, prob_good 사용) ---
    roc_auc = roc_auc_score(eval_df["y_good"], eval_df["prob_good"])

    # --- Task 2: Total Net Profit 계산 ---
    ...
    # --- Task 2: Total Score 계산 (정규화) ---
    part_auc = max(roc_auc - 0.5, 0.0) / 0.5
    part_profit = max(total_net_profit, 0.0) / 20000.0  # 공식에서 분모 20000 고정
...
# ----------------------------------------------------
# 6. 전체 파이프라인 (Validation 60개 고정 Cross Validation)
# ----------------------------------------------------
class ProductionPipeline:
    ...
    def make_cv_splits(self, y_series, n_splits=5, n_ng_val=15, n_good_val=45, base_seed=42):
        """
        동일한 15/45 비율을 유지하면서
        seed만 바꿔가며 여러 fold를 만드는 간단한 cross validation
        """
        splits = []
    ...
    # ---------------- Task1/Task2 공식과 동일한 Validation 점수 계산 ----------------
    @staticmethod
    def calculate_competition_score(y_true, y_prob, k=15, profit_good=100, cost_ng=2000):
        """
        Val 60개( y_true: NG=1, Good=0 )에서
        - NG 확률이 가장 낮은 k개를 decision=True 로 선택
        - Task 1: ROC-AUC Score
        - Task 2: 공식과 동일하게 Total Net Profit, Total Score 계산
        """
        ...
        # --- Task 1: ROC-AUC Score (Good=1 기준) ---
        roc_auc = roc_auc_score(y_good, prob_good)

        # --- decision = True인 k개 선택 (NG 확률 낮은 순) ---
        ...
        profit_comp = max(total_net_profit, 0.0) / 20000.0  # 공식과 동일하게 20000 고정
    ...
    def run_production_pipeline(self):
        ...
        # 5. Dataset / DataLoader (FeatureEncoder 학습용)
        ...
        # 9. Cross Validation 설정 (각 fold마다 Val 60개: NG 15, Good 45)
        ...
        print(f"\nCross Validation with {self.n_cv_splits} folds (each val: 60 samples, NG=15, Good=45)")
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            # 🔥 Task1/Task2 공식과 동일한 validation 스코어 사용
            roc, profit, score = self.calculate_competition_score(
                y_true=y_val,
                y_prob=val_prob_ng,
                k=15,
                profit_good=100,
        ...
        # 12. 제출 파일 생성 (대회 스펙: L/P 반 갈라서 각 170개 선택)
        ...
        submission.to_csv("./data/CNN_MLP_RF_CV_withVal60_v2_submission.csv", index=False)
        print("Saved submission to ./data/CNN_MLP_RF_CV_withVal60_v2_submission.csv")

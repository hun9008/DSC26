...
class ProductionPipeline:
    ...
    # ---------------- 새 Validation 점수 계산 (k=15 선택) ----------------
    @staticmethod
    def calculate_competition_score(y_true, y_prob, k=15, profit_good=100, cost_ng=150):
        """
        Val 60개( NG=1, Good=0 )에서
        - NG 확률이 가장 낮은 k개 선택
        - Good 맞추면 +profit_good, NG 고르면 -cost_ng
        - AUC, Profit, Total Score 계산
        """
        ...
        df_eval = pd.DataFrame({"prob": y_prob, "y_ng": y_true})
        selected = df_eval.nsmallest(k, "prob")  # NG 확률 낮은 k개 = Good일 가능성이 높은 k개
        ...
        profit_comp = max(total_net_profit, 0) / 1500.0  # k=15일 때 Good 15개 전부 맞추면 1500
    ...
    def run_production_pipeline(self):
        ...
        print(f"(k=15 선택, AUC + Profit 기반 점수)")

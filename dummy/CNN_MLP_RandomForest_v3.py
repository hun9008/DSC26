...
def evaluate_score_general(
    ...
    """대회에서 주어진 평가 스코어 계산 함수 (전체 데이터 기준 L/P 반 갈라서 200개씩 선택)
    (참고용으로 남겨둔 함수, 본 파이프라인 평가는 아래 calculate_competition_score 사용)
    """
...
class FeatureEncoder(nn.Module):
    ...
    Feature Encoder:
      ...
      - concat 후 feature 반환(extract_features) 또는 head까지 통과시켜 logit 출력(forward)
    """
...
# ----------------------------------------------------
# 6. 전체 파이프라인 (단일 Validation 60개, NG=15 / Good=45, k=15 평가)
# ----------------------------------------------------
class ProductionPipeline:
    """FeatureEncoder + MainModel 하이브리드 파이프라인"""

    def __init__(self, n_epochs=13, batch_size=32, n_cv_splits=5):
        # n_cv_splits는 더 이상 사용하지 않지만, main() 시그니처 맞추려고 그대로 둠
    ...
    # ---------------- eval_form.py와 동일한 Validation 점수 계산 ----------------
    @staticmethod
    def calculate_competition_score(y_true, y_prob, k=15, profit_good=100, cost_ng=150):
        """
        eval_form.py의 HybridModelPipeline.calculate_competition_score와 동일한 방식:
        - y_true: 0(Good), 1(NG)
        - y_prob: NG 확률
        - k개(15개) 선택, Good +100 / NG -150
        - AUC, Profit을 정규화하여 최종 점수 산출
        반환: total_score, roc_auc, total_net_profit, k
        """
        ...
        selected = df_eval.nsmallest(k, 'prob')  # NG 확률 낮은 k개 선택
        ...
        profit_comp = max(total_net_profit, 0) / 1500.0  # k=15, Good 15개 전부 맞추면 1500
    ...
    def run_production_pipeline(self):
        ...
        # 4. 기본 피처 전처리 준비 (전체 train basic에 대해 fit)
        ...
        # 5. Dataset / DataLoader (FeatureEncoder 학습용) - 전체 train 사용
        train_dataset_full = MultiModalDataset(train_df, X_train_basic_np, self.rasterizer, train_Y_series.values)
        test_dataset = MultiModalDataset(test_df, X_test_basic_np, self.rasterizer, labels_np=None)

        train_loader_full = DataLoader(train_dataset_full, batch_size=self.batch_size, shuffle=True)
        test_loader = DataLoader(test_dataset, batch_size=self.batch_size, shuffle=False)

        # 6. Feature Encoder 학습 (전체 train 기준으로 feature extractor 학습)
        self.train_feature_encoder(train_loader_full)

        # 7. Feature Encoder를 이용해 feature 추출 (전체 train, 전체 test)
        train_loader_seq_full = DataLoader(train_dataset_full, batch_size=self.batch_size, shuffle=False)
        X_train_feat_full = self.extract_features(train_loader_seq_full, is_test=False)
        X_test_feat = self.extract_features(test_loader, is_test=True)

        print(f"Encoded features (Train): {X_train_feat_full.shape}")
        print(f"Encoded features (Test) : {X_test_feat.shape}")

        # 8. 하이브리드 피처 생성 (기본 피처 + FeatureEncoder 피처)
        X_train_hybrid_full = np.concatenate([X_train_basic_np, X_train_feat_full], axis=1)
        X_test_hybrid = np.concatenate([X_test_basic_np, X_test_feat], axis=1)

        print(f"Hybrid features (Train): {X_train_hybrid_full.shape}")
        print(f"Hybrid features (Test) : {X_test_hybrid.shape}")

        # 9. eval_form.py와 동일하게 단일 Validation Set 구성 (NG=15, Good=45)
        ...
        X_train_fold = X_train_hybrid_full[train_idx]
        y_train_fold = train_Y_series.values[train_idx]

        X_val = X_train_hybrid_full[val_idx]
        y_val = train_Y_series.values[val_idx]

        print("\n===== Single Validation (No Cross-Validation) =====")
        print(f"Train size: {X_train_fold.shape[0]}")
        print(f"Val size  : {X_val.shape[0]} (NG={ (y_val==1).sum() }, Good={ (y_val==0).sum() })")
        print("Validation 평가 방식: NG15 / Good45, k=15, 사진 속 최종 공식과 동일")

        # 10. Validation 학습/평가 (RandomForest)
        model = MainModel(n_estimators=200, random_state=42, n_jobs=-1)
        model.fit(X_train_fold, y_train_fold)

        val_prob_ng = model.predict_proba(X_val)[:, 1]

        total_score, roc, profit, k = self.calculate_competition_score(
            y_true=y_val,
            y_prob=val_prob_ng,
            k=15,
            profit_good=100,
        ...
        submission.to_csv("./data/CNN_MLP_RF_evalform_style_submission.csv", index=False)
        print("Saved submission to ./data/CNN_MLP_RF_evalform_style_submission.csv")
...
def main():
    pipeline = ProductionPipeline(n_epochs=13, batch_size=32, n_cv_splits=5)

...
class ProductionPipeline:
    ...
    def optuna_objective(self, trial, X, y, n_folds=3, n_ng_val=15, n_good_val=45, base_seed=42):
        ...
        - n_folds번 반복 CV 수행 후 total_score 평균 리턴 (maximize)
        """
        ...
        for fold in range(n_folds):
            ...
            _, _, total_score = evaluate_score_general(
                y_ng=y_val_fold,
                prob_ng=val_prob_ng,
                n_select_each=200,
                profit_good=100,
    ...
    def run_production_pipeline(self):
        ...
        study.optimize(
            lambda trial: self.optuna_objective(
                ...
                X=X_train_hybrid,
                y=y_array,
                n_folds=3,        # 속도 고려해서 3-fold tuning (원하면 5로 늘리면 됨)
                n_ng_val=15,
                n_good_val=45,
                base_seed=42
            ),
            n_trials=20,          # 시도 횟수 (원하면 늘리기 가능)
        )
        ...
        self.cross_validate(
            ...
            n_ng_val=15,
            n_good_val=45,
            base_seed=100  # tuning때와 seed 다르게 해도 됨
        )
        ...
        submission.to_csv("../data/submission_dummy/CNN_MLP_EnsembleVoting_OptunaCV_submission.csv", index=False)
        print("Saved submission to ../data/submission_dummy/CNN_MLP_EnsembleVoting_OptunaCV_submission.csv")

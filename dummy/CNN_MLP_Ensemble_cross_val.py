...
class MainModel:
    ...
    Main Model:
      ...
      - 평가 시 항상 이 앙상블에 대해 predict_proba 실행
    """
...
class ProductionPipeline:
    ...
    # ---------------- Cross Validation (NG 15 / Good 45 고정) ----------------
    def cross_validate(self, X, y, n_folds=5, n_ng_val=15, n_good_val=45, base_seed=42):
        """
        작은 데이터셋에서 같은 비율(15 NG / 45 Good)을 유지한 채
        seed만 바꿔가며 repeated random CV 형태로 평가.
        ⇒ 각 fold마다 MainModel(앙상블)을 새로 만들어 fit/predict_proba 하므로
           여기서 계산되는 점수는 '앙상블 전체'에 대한 성능.
        """
        fold_results = []
        ...
        for fold in range(n_folds):
            ...
            print(f"Train size: {X_train_fold.shape[0]}, "
                  f"Val size: {X_val_fold.shape[0]} (NG={ (y_val_fold==1).sum() }, Good={ (y_val_fold==0).sum() })")

            # 새 Ensemble 모델로 학습 (앙상블 전체)
            model = MainModel()
            model.fit(X_train_fold, y_train_fold)

            # validation 평가 (앙상블 전체에 대한 predict_proba)
            val_prob_ng = model.predict_proba(X_val_fold)[:, 1]
            roc_auc, total_net_profit, total_score = evaluate_score_general(
                y_ng=y_val_fold,
                prob_ng=val_prob_ng,
                n_select_each=200,   # 대회 스펙 유지
                profit_good=100,
                cost_ng=150
            )
    ...
    def run_production_pipeline(self):
        ...
        # 9. Cross Validation (NG 15 / Good 45, 5 folds)
        self.cross_validate(
            X=X_train_hybrid,
            y=y_array,
            n_folds=5,
        ...
        # 최종 제출에선 170개씩 선택하도록 유지 (원래 코드 기준)
        ...
        submission.to_csv("./data/CNN_MLP_EnsembleVoting_CV_submission.csv", index=False)
        print("Saved submission to ./data/CNN_MLP_EnsembleVoting_CV_submission.csv")

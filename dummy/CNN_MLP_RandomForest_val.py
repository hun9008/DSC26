...
class ProductionPipeline:
    ...
    def run_production_pipeline(self):
        ...
        # 9. Validation 60개 고정 (NG 15, Good 45)
        ...
        X_train_final = X_train_hybrid[train_idx]
        y_train_final = train_Y_series.values[train_idx]

        X_val = X_train_hybrid[val_idx]
        y_val = train_Y_series.values[val_idx]

        print(f"Train size after split: {X_train_final.shape[0]}")
        print(f"Validation size       : {X_val.shape[0]} (NG={ (y_val==1).sum() }, Good={ (y_val==0).sum() })")

        # 10. Main Model 학습 (Validation 평가용)
        self.main_model = MainModel(n_estimators=200, random_state=42, n_jobs=-1)
        self.main_model.fit(X_train_final, y_train_final)

        # 11. Validation 성능 평가 (고정된 60개)
        val_prob_ng = self.main_model.predict_proba(X_val)[:, 1]
        print("\nValidation performance (60 samples, NG:15, Good:45)")
        evaluate_score_general(
            y_ng=y_val,
            prob_ng=val_prob_ng,
            n_select_each=200,   # 여기서는 200이 아니라도 되지만 대회 스펙을 그대로 사용
            profit_good=100,
            cost_ng=150
        )
        ...
        submission.to_csv("./data/CNN_MLP_RF_withVal60_submission.csv", index=False)
        print("Saved submission to ./data/CNN_MLP_RF_withVal60_submission.csv")

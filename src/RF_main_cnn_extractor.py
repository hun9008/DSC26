...
class FullE2EModel(nn.Module):
    ...
    def __init__(self, basic_feature_dim, image_cnn_output_dim=64, basic_mlp_output_dim=64, input_grid_size=64):
...
class ProductionPipeline:
    ...
    def train_cnn_extractor(self, train_loader):
        """E2E 모델을 '학습'시켜 피처 추출기(best_model.pth)를 만듭니다. (Train 전체 사용)"""
        ...
        print("\n[Main] CNN 피처 추출기 학습 시작 (Train 전체 사용)...")
    ...
    def run_production_pipeline(self):
        ...
        # 3. 래스터화 설정 (Cubic 보간법 사용)
        self.rasterizer = SpatialRasterizer(x_min, x_max, y_min, y_max, grid_size=64, interpolation_method='linear')
        ...
        if not os.path.exists(self.encoder_weight_path):
            ...
            self.train_cnn_extractor(train_loader_cnn)
        else:
        ...
        train_loader_seq = DataLoader(train_dataset_cnn, batch_size=self.batch_size, shuffle=False)

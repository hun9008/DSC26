...
# ----------------------------------------------------
# 5. Main Model (CatBoost 기반 이진 분류기)
# ----------------------------------------------------
class MainModel:
    """
    Main Model:
      - 입력: FeatureEncoder에서 추출한 feature (+ 필요시 기본 피처)
    ...
    def __init__(
        ...
        self.model = CatBoostClassifier(
            ...
            verbose=verbose,  # 로그 너무 길어지지 않게
            # GPU 쓰고 싶으면 아래 두 줄 열면 됨 (CUDA 설치돼 있을 때)
            # task_type="GPU",
            # devices="0",
        )

    def fit(self, X, y):
        # CatBoost는 numpy array를 그대로 받아도 됨
        self.model.fit(X, y)

    def predict_proba(self, X):
        # predict_proba: (n_samples, 2) [P(class0), P(class1)]
...
class ProductionPipeline:
    """(사전 학습된) FeatureEncoder + MainModel(CatBoost) 하이브리드 파이프라인"""
    ...
    def run_production_pipeline(self):
        ...
        save_path = f"../data/submission/CNN_CatBoost_submission_{timestamp}.csv"

...
class ProductionPipeline:
    ...
    def __init__(self, n_epochs=13, batch_size=32, n_cv_splits=5,
        ...
        #   개수를 늘리면 더 많은 RF를 앙상블 (학습 시간 증가)
        # self.seed_list = [7, 17, 27, 37, 47, 57, 67, 77, 87, 97, 111]
        # 1~999 까지 1씩 증가
        self.seed_list = [s for s in range(1, 42 + 10)]

        self.seed_weights = np.ones(len(self.seed_list), dtype=float)
    ...
    @staticmethod
    def rank_based_ensemble(prob_list, weights=None):
        """
        prob_list: 길이 M 리스트, 각 원소는 (N,) shape np.ndarray (각 모델의 예측 확률)
        weights : (M,) shape 가중치. None이면 균등 가중치.

        각 모델별 확률을 rank로 변환 후 [0,1]로 정규화 → 가중 평균.
        """
        prob_arr = np.stack(prob_list, axis=0)   # (M, N)
        M, N = prob_arr.shape
        ...
        ranks = np.empty_like(prob_arr, dtype=float)

        for i in range(M):
            p = prob_arr[i]
            # argsort 두 번으로 rank (0 ~ N-1)
            order = p.argsort().argsort().astype(float)
            if N > 1:
                ranks[i] = order / (N - 1)      # 0 ~ 1 사이로 정규화
            else:
                ranks[i].fill(0.5)

        # 가중 평균 rank
        ensemble_scores = np.average(ranks, axis=0, weights=weights)
    ...
    def run_production_pipeline(self):
        ...
        # 10. 제출용 모델 재학습 + 여러 seed RF 앙상블
        test_prob_list = []

        print("\n[Main] Train final models for seed ensemble")
        print(f"[Main] Seeds: {self.seed_list}")

        for s in self.seed_list:
            print(f"[Main]  -> Training RF with random_state={s}")
            model = MainModel(n_estimators=200, random_state=s, n_jobs=-1)
            model.fit(X_train_hybrid, train_Y_series.values)

            prob = model.predict_proba(X_test_hybrid)[:, 1]
            test_prob_list.append(prob)
            print(f"[Main]     Test prob range (seed={s}): {prob.min():.4f} ~ {prob.max():.4f}")

        # 11. Rank-based weighted averaging ensemble
        test_prob_ens = self.rank_based_ensemble(test_prob_list, self.seed_weights)
        print(f"\n[Main] Test prob range (rank-ensemble): {test_prob_ens.min():.4f} ~ {test_prob_ens.max():.4f}")
        ...
        save_path = f"../data/submission/CNN_RF_seedEnsemble_{timestamp}.csv"

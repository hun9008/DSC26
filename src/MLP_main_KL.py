...
class ProductionPipeline:
    """(사전 학습된) FeatureEncoder + MainModel(MLP) 하이브리드 파이프라인"""
    ...
    def train_mlp_with_kl(self, X_train, y_train, X_target, verbose_prefix=""):
        ...
        X_train: (N_s, D) numpy
        y_train: (N_s,) numpy
        X_target: (N_t, D) numpy  (label 없음, 도메인 정렬용: test feature)
        """
    ...
    def run_production_pipeline(self):
        ...
        # 8. 하이브리드 피처 생성 (지금은 encoder feature만 사용)
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            # fold별 MLP + KL 학습 (target 도메인은 항상 전체 test feature)
        ...
        print("\n[Main] Train final MLP with KL (full train)")
        ...
        save_path = f"../data/submission/CNN_MLP_KL_submission_{timestamp}.csv"
...
def main():
    pipeline = ProductionPipeline(
        n_epochs=30,          # 필요시 조절
        batch_size=64,        # 필요시 조절
        n_cv_splits=5,
        encoder_weight_path="../weight/feature_encoder.pth",
        hidden_dim=256,
        dropout=0.2,
        lr=1e-3,
        lambda_align=1e-3,    # KL 비중 (너무 크면 분류 성능 떨어질 수 있음)
    )

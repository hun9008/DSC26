...
# ----------------------------------------------------
# 5. Main Model (SVM 기반 이진 분류기)
# ----------------------------------------------------
class MainModel:
    """
    Main Model:
      - 입력: FeatureEncoder에서 추출한 feature + 기본 피처
      - 모델: SVM(poly, degree=3)
      - 출력: NG 확률 (class=1)
    """
...
class ProductionPipeline:
    """(사전 학습된) FeatureEncoder + MainModel(SVM) 하이브리드 파이프라인"""
    ...
    def run_production_pipeline(self):
        ...
        save_path = f"../data/submission/CNN_SVM_submission_{timestamp}.csv"

...
from lightgbm import LGBMClassifier  # 사용은 안 하지만 import 유지
...
class ProductionPipeline:
    ...
    def run_production_pipeline(self):
        ...
        # 10. 전체 OOF 기준으로 개별 모델 성능 + score 저장
        ...
        for m_idx, name in enumerate(model_names):
            ...
            per_model_scores.append(score_m)
            print(
                f"[OOF] {name:25s} "
                f"ROC={roc_m:.6f} Profit={profit_m:.2f} Score={score_m:.6f}"
            )

        per_model_scores = np.array(per_model_scores)

        # 11. OOF Score 기준 상위 N개 모델 선택
        top_n = min(self.top_n_models, num_models)
        sorted_idx = np.argsort(per_model_scores)[::-1]   # score 내림차순
        selected_idx = sorted_idx[:top_n]

        print("\n===== Top-N models for 2nd-stage Hill Climbing =====")
        for rank, idx in enumerate(selected_idx, start=1):
            print(
                f"[Rank {rank:02d}] {model_names[idx]:25s} "
                f"Score={per_model_scores[idx]:.6f}"
            )

        selected_model_names = [model_names[i] for i in selected_idx]

        # 선택된 모델들에 대해서만 OOF / Test 예측 행렬 재구성
        oof_selected = oof_preds[:, selected_idx]  # (n_train, top_n)
        test_selected_folds = [
            fold_pred[:, selected_idx] for fold_pred in test_preds_folds
        ]  # 각 원소: (n_test, top_n)

        # 12. Hill Climbing으로 상위 N개 모델에 대한 최적 가중치 찾기
        best_w, best_score = hill_climb_ensemble(
            y_true=y_all,
            pred_matrix=oof_selected,
            step=0.05,
            max_iter=200,
            min_weight=0.0,
        )

        # 13. Hill Climbing 앙상블 OOF 성능 + Fold별 요약
        oof_blend = oof_selected @ best_w
        roc_all, profit_all, score_all = calculate_competition_score(
            y_true=y_all,
            y_prob=oof_blend,
        )
        print("\n===== OOF Hill-Climb Ensemble Performance (Top-N models) =====")
        print(f"ROC-AUC = {roc_all:.6f}, Profit = {profit_all:.2f}, Score = {score_all:.6f}")
        print(f"Best weights (Top-{top_n} models):")
        for name, wv in zip(selected_model_names, best_w):
        ...
        for fold_idx, (_, val_idx) in enumerate(cv_splits):
            ...
            print(
                f"[Fold {fold_idx+1}] Blended ROC={roc_f:.6f} "
                f"Profit={profit_f:.2f} Score={score_f:.6f}"
            )

        print("\n===== CV Summary (Hill-Climb Ensemble, Top-N models) =====")
        ...
        # 14. Test 예측: fold별 예측 평균 후 Hill Climb 가중치 적용 (Top-N만 사용)
        test_preds_mean_selected = np.mean(test_selected_folds, axis=0)  # (n_test, top_n)
        test_prob = test_preds_mean_selected @ best_w
        print(f"\n[Main] Test prob range: {test_prob.min():.4f} ~ {test_prob.max():.4f}")

        # 15. 제출 파일 생성 (기존 로직 유지)
        ...
        save_path = f"../data/submission/ensemble_hillclimb_topN_{timestamp}.csv"

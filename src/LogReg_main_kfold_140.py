...
from sklearn.linear_model import LogisticRegression  # ★ 변경: 로지스틱 회귀
...
# ----------------------------------------------------
# 5. Main Model (Logistic Regression 기반 이진 분류기)
# ----------------------------------------------------
class MainModel:
    """
    Main Model:
      - 입력: FeatureEncoder에서 추출한 feature (+ 필요시 기본 피처)
      - 모델: LogisticRegression (L2, C=1)
      - 출력: NG 확률
    """

    def __init__(self, C=1.0, penalty="l2", max_iter=1000,
                 random_state=42, n_jobs=-1):
        # solver='lbfgs' 는 L2 지원 + n_jobs 사용 가능
...
class ProductionPipeline:
    """(사전 학습된) FeatureEncoder + MainModel(LogReg) 하이브리드 파이프라인"""
    ...
    def run_production_pipeline(self):
        ...
        save_path = f"../data/submission/CNN_LogReg_L2C1_submission_{timestamp}.csv"

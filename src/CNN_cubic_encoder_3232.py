...
# ----------------------------------------------------
# 3. Validation Set 분리 함수 (최적 epoch 결정용)
# ----------------------------------------------------
def make_fixed_validation_indices(y_series, n_ng_val=15, n_good_val=45, seed=42):
    """
    Validation set을 고정된 구성으로 분리 (NG=15, Good=45)
    
    Args:
        y_series: 타겟 레이블 시리즈 (Pandas Series)
        n_ng_val: Validation에 포함할 NG 샘플 수 (기본값: 15)
        n_good_val: Validation에 포함할 Good 샘플 수 (기본값: 45)
        seed: 랜덤 시드 (기본값: 42)
    
    Returns:
        train_idx: Train 인덱스 배열
        val_idx: Validation 인덱스 배열
    """
    ...
    if len(ng_idx) < n_ng_val or len(good_idx) < n_good_val:
        raise ValueError(f"Validation에 필요한 샘플 수가 부족합니다. "
                         f"NG: {len(ng_idx)}/{n_ng_val}, Good: {len(good_idx)}/{n_good_val}")

...
class DataProcessor:
    ...
    def load_data(self, train_path="../data/train.csv", test_path="../data/test.csv", val_indices=None):
        ...
        if val_indices is not None:
            ...
            print(f"[EncoderTrain] Validation {len(val_indices)}개 제외, 증강용 데이터: {len(self.train_aug)}개")
        else:
            # 전체 데이터로 증강
...
def train_encoder(n_epochs=100, batch_size=32, save_dir="../weight", val_ratio=0.2, random_state=42):
    ...
    best_save_path = os.path.join(save_dir, f"feature_encoder_aug_smotenc_best_{date_str}.pth")
    final_save_path = os.path.join(save_dir, f"feature_encoder_aug_smotenc_final_{date_str}.pth")
    val_indices_path = os.path.join(save_dir, f"val_indices_aug_smotenc_{date_str}.npy")

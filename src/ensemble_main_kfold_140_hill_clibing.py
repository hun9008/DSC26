...
class ProductionPipeline:
    ...
    def run_production_pipeline(self):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            main_model.fit(X_train_fold, y_train_fold)

            # 2) 앙상블 성능
            val_prob_ng_ens = main_model.predict_proba(X_val)[:, 1]
            roc_ens, profit_ens, score_ens = calculate_competition_score(
                y_true=y_val,
                y_prob=val_prob_ng_ens,
            )

            print(f"[Fold {fold_idx+1}] Ensemble  ROC={roc_ens:.6f} "
                f"Profit={profit_ens:.2f} Score={score_ens:.6f}")

            cv_roc_list.append(roc_ens)
            cv_profit_list.append(profit_ens)
            cv_score_list.append(score_ens)

            # 3) 개별 모델 성능
            # VotingClassifier는 fit 이후 named_estimators_ 속성에 실제 모델 인스턴스를 가진다
            for name, est in main_model.model.named_estimators_.items():
                if not hasattr(est, "predict_proba"):
                    continue

                val_prob_ng = est.predict_proba(X_val)[:, 1]
                roc_m, profit_m, score_m = calculate_competition_score(
                    y_true=y_val,
                    y_prob=val_prob_ng,
                )

                print(f"[Fold {fold_idx+1}] {name:15s} "
        ...
        save_path = f"../data/submission/ensemble_submission_{timestamp}.csv"

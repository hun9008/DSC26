...
from util.eval import (
    evaluate_score_general,          # 안 써도 import 유지
...
class FeatureEncoder(nn.Module):
    ...
    def __init__(self, basic_feature_dim, image_cnn_output_dim=64, basic_mlp_output_dim=32, input_grid_size=64):
        ...
        self.basic_mlp = nn.Sequential(
            nn.Linear(basic_feature_dim, min(basic_feature_dim * 2, 512)),
            nn.ReLU(),
            nn.BatchNorm1d(min(basic_feature_dim * 2, 512)),
            nn.Dropout(0.3),
            nn.Linear(min(basic_feature_dim * 2, 512), basic_mlp_output_dim),
            nn.ReLU()
        )
...
# ----------------------------------------------------
# 5. FeatureEncoder 학습 + E2E inference & submission
# ----------------------------------------------------
def train_encoder(n_epochs=50, batch_size=32, save_dir="../weight"):
    ...
    weight_path = os.path.join(save_dir, f"feature_encoder_{date_str}.pth")
    ...
    X_test_basic = dp.preprocess_basic(test_X_df)

    # ---------------- train/val split ----------------
    y_np = train_Y.values.astype(np.float32)
    ...
    print(f"[EncoderTrain] Train size: {len(train_idx)}, Val size: {len(val_idx)}")

    train_dataset = MultiModalDataset(
        train_df.iloc[train_idx], X_train_basic[train_idx], rasterizer, y_np[train_idx]
    )
    val_dataset = MultiModalDataset(
        train_df.iloc[val_idx], X_train_basic[val_idx], rasterizer, y_np[val_idx]
    )
    test_dataset = MultiModalDataset(
        test_df, X_test_basic, rasterizer, labels_np=None
    )
    ...
    for ep in range(n_epochs):
        ...
        with torch.no_grad():
            for img, basic, y in val_loader:
                ...
                out = model(img, basic)  # (B,1)
                val_logits_list.append(out.detach().cpu().numpy())
                val_labels_list.append(y.detach().cpu().numpy())

        val_logits = np.concatenate(val_logits_list, axis=0).reshape(-1)
        val_labels = np.concatenate(val_labels_list, axis=0).reshape(-1)
        ...
        print(
            ...
            f"Val ROC={roc:.6f} Profit={profit:.2f} Score={score:.6f}"
        )
        ...
        if score > best_score:
            ...
            best_epoch = ep + 1
            print(f"[EncoderTrain] >>> New best model at epoch {best_epoch}: Score={best_score:.6f}")
    ...
    print(f"[EncoderTrain] Saved best encoder weights to: {weight_path}")
    print(f"[EncoderTrain] Best validation Score={best_score:.6f} at epoch {best_epoch}")

    # ---------------- best model로 test inference ----------------
    ...
    with torch.no_grad():
        for img, basic in test_loader:
            ...
            basic = basic.to(device)
            out = model(img, basic)  # (B,1)
            test_logits_list.append(out.detach().cpu().numpy())

    test_logits = np.concatenate(test_logits_list, axis=0).reshape(-1)
    test_prob = 1.0 / (1.0 + np.exp(-test_logits))
    print(f"[EncoderTrain] Test prob range: {test_prob.min():.4f} ~ {test_prob.max():.4f}")

    # ---------------- submission 생성 (CNN_E2E_*.csv) ----------------
    submission = pd.read_csv("../data/submission/sample_submission.csv")
    # L/P 각각 같은 prob 사용 (이전 로직과 동일)
    ...
    # L/P 각각 170개씩 선택 (prob 낮을수록 양품일 가능성 높다고 가정)
    ...
    sub_path = f"../data/submission/CNN_E2E_{timestamp}.csv"

    submission.to_csv(sub_path, index=False)
    print(f"[EncoderTrain] Saved submission to: {sub_path}")

    selected_count = submission["decision"].sum()
    print(f"[EncoderTrain] Total selected products: {selected_count}")

    logger.close()
    sys.stdout = sys.__stdout__
    print(f"[Main] Log saved to: {logger.log_path}")

    return submission, sub_path


if __name__ == "__main__":
    # 필요하면 n_epochs 조절해서 호출
    submission_df, sub_path = train_encoder(
        n_epochs=50,
        batch_size=32,
        save_dir="../weight"
    )
    ...
    print(f"\nSaved submission path: {sub_path}")